...
export function AlertsIconSheet({
  highlight,
  children,
}: {
  highlight?: boolean;
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        ...
        >
          <Bell size={ICON_SHEET_SIZE} className={iconClassName} aria-hidden />
        </button>
      </SheetTrigger>
      <SheetContent
      ...
      >
        ...
        {children ?? (
          <p className="text-grey-600 dark:text-grey-300 text-sm">No alerts configured.</p>
        )}
      </SheetContent>
    </Sheet>
  );
